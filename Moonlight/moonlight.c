...
// Lit un entier au clavier. Si l'utilisateur tape n'importe quoi,
// on vide le buffer et on renvoie 0 (aucun choix valide).
int     read_choice(void)
{
    int value;
    int c;
...
void    in_house(void)
{
    ...
    {
        type_text("Ce n'est pas une option valide.\n");
        sleep(1);
        in_house();
    }
}
...
void    deep_forest_left(void)
{
    ...
    else if (choix == 1 && shovel == 0)
    {
        type_text("Tu voudrais utiliser une pelle... mais tu n'en as pas.\n");
        type_text("Tes mains nues ne peuvent rien contre elle.\n");
        sleep(3);
        death();
    }
    else
...
void    ending(void)
{
    type_text("Bien joue !! Je sais que le jeu est mal foutu et que l'histoire\n");
    type_text("a des enormes defauts mais faut pas m'en vouloir j'etais au college\n");
    type_text("Bref, Merci d'avoir joue\n");
    sleep(3);
